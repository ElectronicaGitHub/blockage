...
//        vector<Entity*> passive_entities = EntityManager::getNearestPassiveEntities(entity);
        ...
        Size size = render_comp1->animatedSprite->getContentSize();
        
        float posX = render_comp1->animatedSprite->getPositionX();
        float posY = render_comp1->animatedSprite->getPositionY();

        float rect1MinX = posX - size.width  / 2;
        float rect1MaxX = posX + size.width  / 2;
        float rect1MinY = posY - size.height * 1 / 3;
        float rect1MaxY = posY + size.height * 2 / 3;

        Rect r1bottomLine = Rect(rect1MinX + controlPanelSize, rect1MinY - controlPanelSize, size.width - 2 * controlPanelSize, controlPanelSize);
        Rect r1topLine =    Rect(rect1MinX + controlPanelSize, rect1MaxY                   , size.width - 2 * controlPanelSize, controlPanelSize);
        
        Rect r1leftLine =   Rect(rect1MinX,                    rect1MinY + controlPanelSize, controlPanelSize, size.height - 2 * controlPanelSize);
        Rect r1rightLine =  Rect(rect1MaxX,                    rect1MinY + controlPanelSize, controlPanelSize, size.height - 2 * controlPanelSize);
        ...
        for (int j = 0; j < passive_entities.size(); j++) {
            if (EntityManager::entityHasComponent(passive_entities[j], PASSIVE_COLLISION_COMPONENT)) {
                ...
                if (r1bottomLine.intersectsRect(rect2)) {
                    position->y = rect2MaxY + size.height * 1 / 3;
